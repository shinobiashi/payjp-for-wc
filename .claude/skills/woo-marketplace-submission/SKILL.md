...
これは非独占製品（他チャネルでも販売可）の標準レート。独占販売の場合は
...
Wooマーク（WooCommerce, Woo等）を製品名・ブランド名に組み込むことは不可。
「WooCommerce Product Name」のように先頭に置く形式は不可で、
「Product Name for WooCommerce」形式のみ許可される。
...
実装が必要。サンドボックスアクセスはビジネスレビュー通過後に付与され、
資格情報（APIキー/シークレット）は Vendor Dashboard で発行される。
Billing API は月額・年額の両方の課金間隔に対応し、プラン切替（アップグレード/
ダウングレード）フローも提供される。

```
月額サブスクリプション / SaaS の場合:
→ SaaS Billing API 実装が必要
→ サンドボックスはビジネスレビュー通過後に付与
→ APIキー/シークレットは Vendor Dashboard で発行
...
5. Activation / Security / Malware の合格後、自動的にデプロイされる
...
- WooCommerce.com のヘルプデスク経由でチケットが来る（製品固有の質問はベンダーのメールへ転送される）
- 24時間以内の初回応答を目標（公式要件は「timely responses」）
...
（旧 woocommerce.com/document/ の該当ページはリダイレクトされる）:
