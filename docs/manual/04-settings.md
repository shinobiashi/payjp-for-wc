...
「**通知**」の「**異常通知メールアドレス**」は、決済に異常が起きたときの通知メールの宛先です。

- 例えば「キャンセル済みの注文に対して、あとから支払い完了の連絡が届いた」といったイレギュラーを検知すると、このアドレスに通知メールが届きます（詳しくは [第7章 7-7](07-daily-operations.md#7-7-異常があったときの通知メール) を参照）。
- **空欄のままで OK です。** 空欄の場合は、サイト管理者のメールアドレス（「設定」→「一般」の管理者メールアドレス）に届きます。
- 経理担当者など別の宛先で受け取りたい場合だけ入力してください。

## 4-6. デバッグ（デバッグログ）

「**デバッグ**」の「**デバッグログ**」は、PAY.JP との通信内容を記録する機能です。

- **普段はチェックを外したまま（オフ）にしてください。**
- 決済がうまく動かないなどのトラブルを調査するときだけオンにします。記録されたログは「WooCommerce」→「ステータス」→「ログ」で確認できます（詳しくは [第8章](08-faq.md#調査のためのログについて) を参照）。
- 調査が終わったらオフに戻してください。

## 4-7. Webhook（ウェブフック）を設定する
...
4. PAY.JP ダッシュボードに表示される「**Webhook Secret**（認証用の合言葉）」をコピーします。
5. プラグイン設定画面に戻り、「**Webhook シークレット**」欄に貼り付けます。
